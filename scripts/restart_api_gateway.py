...
def find_and_kill_api_gateway():
    """Find and kill any running API Gateway processes"""
    count = 0
    
    print("Looking for running API Gateway processes...")
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            cmdline = proc.info.get('cmdline', [])
            if cmdline and len(cmdline) > 2:
                # Look for uvicorn processes running the API Gateway
                if 'uvicorn' in cmdline[0].lower() and 'main:app' in cmdline and 'api-gateway' in ' '.join(cmdline):
                    print(f"Found API Gateway process (PID: {proc.info['pid']})")
                    proc.terminate()
                    count += 1
                    ...
                    try:
                        ...
                        print(f"Process {proc.info['pid']} didn't terminate gracefully, forcing...")
                        proc.kill()
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            pass
    
    if count == 0:
        print("No running API Gateway processes found.")
    else:
        print(f"Terminated {count} API Gateway processes.")
        # Give processes time to fully terminate
        time.sleep(2)

def main():
    """
    Restart the API Gateway service with simulation mode enabled by default.
    This ensures the service can run without requiring actual cloud credentials.
    """
    parser = argparse.ArgumentParser(description="Restart the Cloud Optimizer API Gateway")
    ...
    # First, try to stop any running API Gateway
    find_and_kill_api_gateway()
